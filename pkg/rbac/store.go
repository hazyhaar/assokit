...
//
// La détection de cycle et l'insertion sont enveloppées dans une transaction : sous
// pool mono-connexion, deux statements séparés sur s.DB relâchent la connexion entre
// la lecture des ancêtres et l'INSERT, ouvrant une fenêtre TOCTOU où deux AddInherit
// opposés concurrents (A→B et B→A) passent chacun le contrôle avant l'insert de
// l'autre et forment un cycle. La transaction tient la connexion sur tout le contrôle.
func (s *Store) AddInherit(ctx context.Context, childID, parentID string) error {
	...
	if err != nil {
		return fmt.Errorf("rbac: add inherit begin: %w", err)
	}
	defer tx.Rollback() //nolint:errcheck

	// Ancêtres de parentID DANS la transaction : si childID en fait partie, l'arête
	// child→parent fermerait un cycle.
	...
	if err != nil {
		return fmt.Errorf("rbac: add inherit ancestors: %w", err)
	}
	for rows.Next() {
		var id string
	...
	if err := tx.Commit(); err != nil {
		return fmt.Errorf("rbac: add inherit commit: %w", err)
	}
