...
func New(db *sql.DB, signingKeyBytes []byte, userProvider UserProvider, idGen IDGenerator) *Storage {
...
func (s *Storage) populateUserinfo(ctx context.Context, userinfo *oidc.UserInfo, userID string, scopes []string) error {
	u, err := s.userProvider.GetUser(ctx, userID)
	if err != nil {
		return fmt.Errorf("userinfo: user %s: %w", userID, err)
	}
	if u == nil {
		return fmt.Errorf("userinfo: %w: user %s", ErrNotFound, userID)
	}
	...
	// Inclure les permissions effectives comme claims privés
	if err := s.injectPermissions(ctx, userinfo, userID, scopes); err != nil {
		// non-fatal
		_ = err
	}
	return nil
...
func (s *Storage) injectPermissions(ctx context.Context, userinfo *oidc.UserInfo, userID string, scopes []string) error {
	perms, err := s.userProvider.ListPermissions(ctx, userID)
	if err != nil {
		return err
	}
	if len(perms) > 0 {
