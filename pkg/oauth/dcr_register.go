...
// Register insère un nouveau client en DB. Génère client_id via idGen.
// Public client → client_secret vide. Confidential → secret 32 bytes hex.
func Register(ctx context.Context, db *sql.DB, req RegisterRequest, idGen IDGenerator) (*RegisterResponse, error) {
...
// DCRClientTTL : un client auto-enregistré (DCR, owner_user_id vide) est
// considéré abandonné s'il a été créé il y a plus de cette durée sans posséder
// le moindre token actif (non révoqué, non expiré). claude.ai et les clients
// MCP re-register via DCR à la demande, donc purger les souches mortes est sûr.
const DCRClientTTL = 30 * 24 * time.Hour

// PurgeStaleClients supprime les clients DCR périmés (créés avant now-TTL, sans
// token actif). N'affecte JAMAIS les clients possédant un owner_user_id (clients
// confidentiels créés par un humain) ni ceux avec un token vivant. Retourne le
// nombre de clients supprimés. Idempotent, sûr à appeler souvent.
func PurgeStaleClients(ctx context.Context, db *sql.DB, now time.Time) (int64, error) {
	cutoff := now.Add(-DCRClientTTL).UTC().Format(time.RFC3339)
	...
	`, cutoff, nowStr)
	if err != nil {
		return 0, fmt.Errorf("dcr: purge stale clients: %w", err)
	}
