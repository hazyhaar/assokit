...
func TestRegisterAuthenticateRoundtrip(t *testing.T) {
	...
	u, err := s.Register(ctx, "boris@nps.fr", "secret123", "L'administrateur")
	if err != nil {
		t.Fatalf("Register: %v", err)
	}
	if u.ID == "" || u.Email != "boris@nps.fr" {
	...
	u2, err := s.Authenticate(ctx, "boris@nps.fr", "secret123")
	if err != nil {
...
func TestEmailNormalized(t *testing.T) {
	...
	s.Register(ctx, "  BORIS@NPS.FR  ", "pass", "L'administrateur")
	_, err := s.Authenticate(ctx, "boris@nps.fr", "pass")
	if err != nil {
