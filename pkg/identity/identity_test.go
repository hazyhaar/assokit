...
func TestRegisterAuthenticateRoundtrip(t *testing.T) {
	...
	u, err := s.Register(ctx, "boris@example.org", "secret123", "L'administrateur")
	if err != nil {
		t.Fatalf("Register: %v", err)
	}
	if u.ID == "" || u.Email != "boris@example.org" {
	...
	u2, err := s.Authenticate(ctx, "boris@example.org", "secret123")
	if err != nil {
...
func TestAuthenticateWrongPassword(t *testing.T) {
	...
	s.Register(ctx, "test@example.org", "correct", "Test")
	_, err := s.Authenticate(ctx, "test@example.org", "wrong")
...
func TestAuthenticateUnknownEmail(t *testing.T) {
	...
	_, err := s.Authenticate(ctx, "unknown@example.org", "pass")
	if !errors.Is(err, identity.ErrInvalidCredentials) {
		t.Errorf("want ErrInvalidCredentials, got %v", err)
	}
}
...
func TestRegisterEmailTaken(t *testing.T) {
	...
	s.Register(ctx, "dup@example.org", "pass", "First")
	_, err := s.Register(ctx, "dup@example.org", "pass2", "Second")
...
func TestEmailNormalized(t *testing.T) {
	...
	s.Register(ctx, "  BORIS@EXAMPLE.ORG  ", "pass", "L'administrateur")
	_, err := s.Authenticate(ctx, "boris@example.org", "pass")
	if err != nil {
...
func TestGetByID(t *testing.T) {
	...
	u, _ := s.Register(ctx, "get@example.org", "pass", "GetUser")
	u2, err := s.GetByID(ctx, u.ID)
	if err != nil || u2 == nil {
		t.Fatalf("GetByID: %v", err)
	}
	if u2.Email != "get@example.org" {
...
func TestAddRemoveRole(t *testing.T) {
	...
	u, _ := s.Register(ctx, "role@example.org", "pass", "RoleUser")
...
func TestMailerNilNocrash(t *testing.T) {
	db := newTestDB(t)
	s := &identity.Store{DB: db, Mailer: nil}
	ctx := context.Background()

	_, err := s.Register(ctx, "nomailer@example.org", "pass", "NoMailer")
	if err != nil {
