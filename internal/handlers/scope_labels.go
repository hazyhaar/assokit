// CLAUDE:SUMMARY Mapping scope/perm → libellé naturel français pour page consent OAuth (migré depuis pkg/horui/auth/pages, vague 3).
...
// libellesScope mappe une slice de scopes en libellés naturels FR.
// Un scope non mappé apparaît comme "Action technique : <scope>" (closed-world).
func libellesScope(scopes []string) []string {
	out := make([]string, len(scopes))
	for i, s := range scopes {
		if lib, ok := scopeLabelsFR[s]; ok {
			out[i] = lib
		} else {
			out[i] = "Action technique : " + s
		}
	}
	return out
}
